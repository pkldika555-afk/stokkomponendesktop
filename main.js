...
const STORAGE_IMAGES_PATH = path.join(app.getPath('userData'), 'storage', 'images');
const STORAGE_BACKUP_PATH = path.join(app.getPath('userData'), 'storage', 'backup');
...
function waitForPort(port, timeout = 60000) {
  return new Promise((resolve, reject) => {
    ...
    const check = () => {
      ...
      client.on('error', () => {
        client.destroy();
        if (Date.now() - start > timeout) reject(new Error(`Port ${port} timeout setelah ${timeout}ms`));
...
function setupEnv() {
  ...
  if (env.includes('STORAGE_IMAGES_PATH=')) {
    env = env.replace(/STORAGE_IMAGES_PATH=.*/, `STORAGE_IMAGES_PATH=${imagesPathForEnv}`);
  } else {
    env += `\nSTORAGE_IMAGES_PATH=${imagesPathForEnv}`;
  }

  if (env.includes('STORAGE_BACKUP_PATH=')) {
    env = env.replace(/STORAGE_BACKUP_PATH=.*/, `STORAGE_BACKUP_PATH=${backupPathForEnv}`);
  } else {
...
async function startMySQL() {
  ...
  if (!fs.existsSync(path.join(MYSQL_DATA, 'mysql'))) {
    ...
    try {
      execSync(`"${MYSQL_BIN}" --initialize-insecure --datadir="${MYSQL_DATA}" --console`, { timeout: 60000, stdio: 'pipe' });
  ...
  try {
    execSync(`"${MYSQL_CLI}" -u root --port=${MYSQL_PORT} -e "CREATE DATABASE IF NOT EXISTS \`${DB_NAME}\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"`, { timeout: 10000 });
...
async function firstTimeSetup() {
  ...
  const migrate = spawnSync(PHP, ['artisan', 'migrate', '--force'], { cwd: WWW, timeout: 60000, stdio: 'pipe' });
  ...
  if (!fs.existsSync(storageTarget)) fs.mkdirSync(storageTarget, { recursive: true });
  if (!fs.existsSync(storagePath)) {
    try {
      fs.symlinkSync(storageTarget, storagePath, 'junction');
      console.log('[Setup] storage symlink OK');
    } catch(e) {
      spawnSync(PHP, ['artisan', 'storage:link'], { cwd: WWW, timeout: 15000, stdio: 'pipe' });
