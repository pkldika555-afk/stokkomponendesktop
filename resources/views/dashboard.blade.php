    ...
    <body>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
            ...
            <div class="bg-gray-900 border {{ $stokRendah > 0 ? 'border-rose-900/50' : 'border-gray-800' }} rounded-xl p-4">
                ...
                <p class="text-2xl font-bold {{ $stokRendah > 0 ? 'text-rose-400' : 'text-gray-400' }}">{{ $stokRendah }}
                </p>
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
            <div class="px-5 py-3.5 border-b border-gray-800 flex items-center justify-between">
                ...
                @if($stokRendah > 0)
                    <span
                        class="flex items-center gap-1.5 text-xs text-rose-400 bg-rose-500/10 border border-rose-500/20 px-2.5 py-1 rounded-lg">
                ...
                <thead>
                    <tr class="border-b border-gray-800">
                        <th class="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                            Komponen</th>
                        <th class="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Tipe
                        </th>
                        <th class="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Lokasi
                        </th>
                        <th class="text-right px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Stok
                        </th>
                        <th class="text-right px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Min.
                        </th>
                        <th class="text-center px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                            Status</th>
                        <th class="text-center px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Aksi
                        </th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-800/60">
                    @forelse($komponen as $k)
                        ...
                        <tr class="hover:bg-gray-800/30 transition-colors {{ $rendah ? 'bg-rose-950/5' : '' }}">
                            ...
                            <td class="px-5 py-3.5">
                                <span
                                    class="text-xs text-gray-400 bg-gray-800 px-2 py-0.5 rounded-md">{{ strtoupper($k->tipe) }}</span>
                            </td>
                            ...
                            <td class="px-5 py-3.5 text-right">
                                <span
                                    class="font-mono font-bold text-sm {{ $habis ? 'text-rose-400' : ($rendah ? 'text-amber-400' : 'text-gray-200') }}">
                            ...
                            <td class="px-5 py-3.5 text-center">
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-lg border text-xs font-medium {{ $statusColor }}">
                                    {{ $statusLabel }}
                                </span>
                            </td>
                            <td class="px-5 py-3.5 text-center">
