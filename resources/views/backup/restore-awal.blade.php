...
<head>
    ...
    <style>
        ...
        .glow-blob { position: fixed; border-radius: 50%; filter: blur(120px); opacity: 0.07; pointer-events: none; }
        .glow-blob-1 { width: 600px; height: 600px; background: radial-gradient(circle, #f59e0b, transparent); top: -200px; right: -200px; }
        .input-field {
...
<body class="bg-slate-950 text-slate-300 min-h-screen flex items-center justify-center px-4">
    ...
    <div class="w-full max-w-sm">
        ...
        <div class="flex flex-col items-center mb-8 animate-fadeup">
            <span class="w-14 h-14 rounded-2xl bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center shadow-lg shadow-amber-500/40 mb-4">
        ...
        <div class="login-card rounded-2xl p-8 animate-fadeup delay-1">
            ...
            <form action="{{ route('restore.awal') }}" method="POST" enctype="multipart/form-data" onsubmit="return confirmRestore()">
                @csrf
                ...
                <label for="backup_file"
                    ...
                    id="dropzone">
                    <i class="ri-file-upload-line text-3xl text-gray-600 group-hover:text-amber-400 transition-colors"></i>
                    ...
                    <input type="file" id="backup_file" name="backup_file" accept=".json" class="hidden" onchange="updateDropzone(this)">
                </label>
